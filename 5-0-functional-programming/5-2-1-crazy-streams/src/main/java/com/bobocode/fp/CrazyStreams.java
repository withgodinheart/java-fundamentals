...
public class CrazyStreams {
    ...
    public List<Account> findAccountsByBirthdayMonth(Month birthdayMonth) {
        return accounts.stream()
                .filter(a -> a.getBirthday().getMonth().equals(birthdayMonth)).toList();
        // ? ASK: What is the difference between toList() and collect(toList());
    }
...
//        return accounts.stream().collect(Collectors.groupingBy(a -> a.getEmail().substring(a.getEmail().indexOf("@") + 1)));
        return accounts.stream()
...
//                .map(Account::getBalance)
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        ...
        // ? ASK: ??? why is not working without mapToObj
    }
